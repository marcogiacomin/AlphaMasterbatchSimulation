...
path_folder_statini = "C:/Users/HP/Desktop/docCod/OP_complete/*"
path_progprod = "C:/Users/HP/Desktop/docCod/PRGPRODnew.txt"


def func_OP(path_folder_statini, path_progprod, secondi_cambio_tool,
            secondi_carico):
    ...
    op = open(path_progprod)
    for line in op:
        list_line = line.split(';')
        idx = 0
        for key in dict_op:
            dict_op[key].append(list_line[idx])
            idx += 1

    dict_op['ordine'] = list(np.array(dict_op['ordine']).astype(int))
    ...
    df_op = pd.DataFrame.from_dict(dict_op)
    ...
    df_stat = df_stat.merge(df_op, on=['estrusore', 'statino']).sort_values(
        by=['estrusore', 'ordine'], ascending=True)
...
a = func_OP(path_folder_statini, path_progprod, 10, 15)
